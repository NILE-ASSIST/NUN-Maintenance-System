...
//   bool _isSubmitting = false; // Add this state variable
...
//       // 1. Prepare the data
//       // Note: For the attachment, you would typically upload to Firebase Storage 
//       // first, get the URL, and add it here. For now, we save the text data.
//       final ticketData = {
//         'description': _detailsController.text.trim(),
//         'category': _category,
//         'location': '${_buildingController.text.trim()}, Room ${_roomController.text.trim()}',
//         'status': 'Pending', // Initial status for Facility Manager to review
//         'dateCreated': FieldValue.serverTimestamp(),
//         'issuerID': user.uid,
//         'issuerEmail': user.email,
//         'attachmentName': _attachmentName, // Just the name for now
//       };

//       // 2. Write to Firestore 'tickets' collection
...
class _ComplaintFormPageState extends State<ComplaintFormPage> {
  ...
  String _category = 'Select a category';
  String? _attachmentName;
  String? _attachmentPath; // Stores the local path of the image
  ...
  Widget _buildCategoryField() {
    return Column(
      ...
      children: [
        const Text(
          'Category',
          style: TextStyle(fontWeight: FontWeight.w800, color: Color(0xFF1F312B)),
        ),
        const SizedBox(height: 8),
        CategoryDropdown(
...
bool _isModernIOS() {
  try {
    ...
    if (digits.isEmpty) return false;
    final major = digits.first;
    return major >= 13; // Changed to 13 as 26 was likely a typo in original code
  } catch (_) {
    return false;
  }
}