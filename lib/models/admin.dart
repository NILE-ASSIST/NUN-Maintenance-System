...
class AdminController {
  ...
  Future<int> countPendingComplaints() async{
    // int totalComplaints = 0;
    final firestore = FirebaseFirestore.instance;
    final snapshot = await firestore.collection('complaints').where('status', isEqualTo: 'pending').count().get();
  ...
  Future<int> countResolvedComplaints() async{
    // int totalComplaints = 0;
    final firestore = FirebaseFirestore.instance;
    final snapshot = await firestore.collection('complaints').where('status', isEqualTo: 'resolved').count().get();
    return snapshot.count ?? 0;
    // return totalComplaints;
  }
  ...
  Widget build(BuildContext context) {
    return Expanded(
      child: Container(
        ...
        child: Column(
          children: [
            Text(label, style: TextStyle(color: textColor, fontSize: 12), textAlign: TextAlign.center),
            ...
            if (futureValue != null)
              FutureBuilder<int>(
                ...
                builder: (context, snapshot) {
                  ...
                  if (snapshot.hasError) return Text('-', style: TextStyle(color: textColor, fontSize: 20, fontWeight: FontWeight.bold));
                  
                  return Text('${snapshot.data ?? 0}', style: TextStyle(color: textColor, fontSize: 20, fontWeight: FontWeight.bold));
                },
              )
            // else
